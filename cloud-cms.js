...
window.rkdCloud={
 ...
 async getSettings(){const {data,error}=await sb.from("site_settings").select("*").eq("id","site").maybeSingle();if(error)throw error;const d=data&&data.data?data.data:{};return Object.assign({id:data?.id||null},d)},
 async saveSettings(v){await this.requireUser();const data={eyebrow:v.eyebrow||"",title:v.title||"",heroCopy:v.heroCopy||"",name:v.name||"",role:v.role||"",about:v.about||"",aboutSecond:v.aboutSecond||"",email:v.email||"",phone:v.phone||"",profilePhoto:v.profilePhoto||"",coverPhoto:v.coverPhoto||""};const {data:row,error}=await sb.from("site_settings").upsert({id:"site",data,updated_at:new Date().toISOString()},{onConflict:"id"}).select().single();if(error)throw error;return row},
