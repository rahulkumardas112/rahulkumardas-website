...
window.rkdCloud={
 ...
 async getPhotos(){const {data,error}=await sb.from("website_photos").select("*").order("id",{ascending:false});if(error)throw error;return (data||[]).filter(p=>p.image_url).map(p=>({...p,description:p.description||"",location:p.location||"",photo_date:p.photo_date||"",featured:!!p.featured}))},
 async savePhoto(p,id){await this.requireUser();if(!p.image&&!id)throw new Error("Photo URL is missing.");const row={title:p.title||"",category:p.category||"Other",image_url:p.image||"",file_name:p.fileName||"",description:p.description||"",location:p.location||"",photo_date:p.photoDate||null,featured:!!p.featured};const q=id?await sb.from("website_photos").update(row).eq("id",id).select().single():await sb.from("website_photos").insert(row).select().single();if(q.error)throw q.error;return q.data},
