...
function renderBooks(){
 ...
 grid.innerHTML=getBooks().filter(b=>(b.status||"Published")==="Published").map((b,i)=>`<article class="product-card"><div class="cover">${bookCover(b)}</div><div class="product-body"><h3>${b.title}</h3><p>${b.desc}</p><span class="price">₹${b.price}</span><div class="card-actions"><a class="small-btn" href="book.html?i=${i}">View</a><a class="small-btn primary" href="${b.buyLink||("checkout.html?i="+i)}">Buy Ebook</a></div></div></article>`).join("");
}
